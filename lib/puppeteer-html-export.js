...
export default class PuppeteerHtmlExport {
  ...
  async waitGoToDomContentLoaded(page, content) {
    ...
    console.log("content---------------")
    console.log(content)
    console.log({ waitUntil: ["domcontentloaded", "networkidle0"], ...timeout })
    console.log(page.goto)

    if (isUrl(content)) {
      //http:// 与 :3000中间的ip地址修改为localhost
      let newUrl = content.replace(/^http:\/\/[^/]+\//, "http://localhost:3000/");
      console.log("newUrl",newUrl)
      await page.goto(newUrl, { waitUntil: ["domcontentloaded", "networkidle0"], ...timeout });
    } else {
  ...
  async htmlContent(content, options = {}) {
    ...
    // 获取指定元素的 HTML 内容
    const html = await page.$eval("#template", (element) => element.innerHTML);
    await this.closeBrowserIfNeeded();
    return html;
  }
